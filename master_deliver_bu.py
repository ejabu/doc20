...
class master_deliver(models.Model):
    ...
    def _get_external(self):
        for record in self:
            #_compute ntuk parent. kalau ada parent yang is_history, dicuekin aja
            ...
            #_compute ntuk childs
                ...
                if self._context.get('rev_num') is not None:
    ...
    def create(self, vals):
        #Conditional if masuk melalui menu item Update
        if self._context.get("update") is not None:
        #   Hanya untuk Create yang dalam One2many
            if vals['is_history']:
                if vals['version_id']:
            ...
            if self._context.get("update") == "status":
                #kenapa mesti di context_to_pass ?
                #karena walaupun sudah pakai VALS, tetap akan di overwrite oleh onchange history_ids untuk create pertama kali
                context_to_pass={
                    'external_status': vals['external_status'],
                    'status_date': vals['status_date'],
                }
                #jika sudah lebih dari satu entry, berarti artinya history_ids sudah bisa di akses
                if len(parent_obj.history_ids)>0:
                    #berarti perlu mengabil latest status
                    import ipdb; ipdb.set_trace()
                    context_to_pass['rev_num'] = parent_obj.rev_num.id
                    context_to_pass['revision_date'] = parent_obj.revision_date
                    vals['rev_num'] = parent_obj.rev_num.id
                    vals['revision_date'] = parent_obj.revision_date
